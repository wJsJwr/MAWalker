...
public class GetConfig {
	public static void parse(Document doc) throws Exception {
		try {
			...
			Info.Profile = Integer.parseInt(xpath.evaluate("/config/profile",
					doc));
			...
			case 1:
				...
				Info.MinAPOnly = xpath.evaluate("/config/option/min_ap_only",
						doc).equals("1");
				...
				Info.debug = xpath.evaluate("/config/option/debug", doc)
						.equals("1");
				Info.nightModeSwitch = xpath.evaluate(
				...
				Info.receiveBattlePresent = xpath.evaluate(
						...
						"1");
				Info.GoStop = xpath.evaluate("/config/option/go_stop", doc)
						.equals("1");
				...
				Info.PublicFairyBattle.BC = Integer.parseInt(xpath.evaluate(
				...
				Info.PrivateFairyBattleNormal.BC = Integer
						.parseInt(xpath
								.evaluate(
				...
				Info.PrivateFairyBattleRare.No = xpath.evaluate(
						...
						doc);
				Info.PrivateFairyBattleRare.BC = Integer
						.parseInt(xpath
								.evaluate(
