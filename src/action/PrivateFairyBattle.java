...
public class PrivateFairyBattle {
	...
	public static boolean run() throws Exception {
		...
		try {
			...
			// Process.info.fairy.FairyName =
			// xpath.evaluate("//battle_vs_info/player[last()]/name", doc);
			...
			// 检查觉醒
			if ((boolean) xpath.evaluate("count(//ex_fairy/rare_fairy)>0", doc,
					XPathConstants.BOOLEAN)) {
				// Yes
				Process.info.fairy.Type = FairyBattleInfo.PRIVATE
				...
				Process.info.fairy.fairyCurrHp = Integer.parseInt(xpath
						.evaluate("//ex_fairy/rare_fairy/hp", doc));
				Process.info.fairy.fairyMaxHp = Integer.parseInt(xpath
						.evaluate("//ex_fairy/rare_fairy/hp_max", doc));
