...
public class GetRewardBox {
	...
	public static void list() throws Exception {
		Document doc;
		try {
			response = Process.network.ConnectToServer(URL_LIST_REWARD_BOX, new ArrayList<NameValuePair>(), false);
		} catch (Exception ex) {
	...
	public static void parse(Document doc) throws NumberFormatException, XPathExpressionException {
		...
		int boxCount = ((NodeList)xpath.evaluate("//rewardbox_list/rewardbox", doc, XPathConstants.NODESET)).getLength();
	...
	public static void get() throws Exception {
		...
		try {
			response = Process.network.ConnectToServer(URL_GET_REWARD_BOX, post, false);
		} catch (Exception ex) {
			ErrorData.currentDataType = ErrorData.DataType.text;
			ErrorData.currentErrorType = ErrorData.ErrorType.ConnectionError;
